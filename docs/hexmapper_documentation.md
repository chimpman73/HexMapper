...
- **LayerPalette Toolset**: The left-hand panel provides a unified interface for all drawing actions. It includes a consistent "Actions" row (Select, Move, Highlight, Erase), specialized layer tools (e.g. Coastline editor, River brush dropdowns, Grid Properties like orientation, coordinate overlays, and grid color), and the primary "Brushes" section for selecting hex stamps.
